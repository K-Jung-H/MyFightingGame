...
public class StageBakerWindow : EditorWindow
{
    ...
    private void ConvertMaterialsToDither(GameObject root, string saveDirectory, List<string> excludedPaths, float near, float far)
    {
        ...
        foreach (MeshRenderer renderer in renderers)
        {
            ...
            for (int i = 0; i < originalMaterials.Length; i++)
            {
                ...
                string safeName = origMat.name.Replace(" (Instance)", "").Trim();
                string assetPath = saveDirectory + "/" + safeName + "_Dither.mat";

                if (materialCache.TryGetValue(safeName, out Material cachedMat))
                {
                ...
                else
                {
                    ...
                    if (existingMat != null)
                    {
                        EditorUtility.CopySerialized(newMat, existingMat);
                        newMat = existingMat; 
                    ...
                    materialCache.Add(safeName, newMat);
    ...
    private Material CreateDitherMaterial(Material original, Shader ditherShader, float near, float far)
    {
        ...
        if (original.HasProperty("_Color") && newMat.HasProperty("_BaseColor") && !original.HasProperty("_BaseColor"))
        {
            newMat.SetColor("_BaseColor", original.GetColor("_Color"));
        }

        if (original.HasProperty("_MainTex") && newMat.HasProperty("_BaseMap") && !original.HasProperty("_BaseMap"))
        {
            newMat.SetTexture("_BaseMap", original.GetTexture("_MainTex"));
            newMat.SetVector("_BaseMap_ST", original.GetVector("_MainTex_ST"));
        }
